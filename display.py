...
def streaming_flash(a, args):
    ...
    while i < 100000:
        ...
        for id in panel_ids:
            if index == 0:
                stream.panel_set(id, RED[0], RED[1], RED[2], transition_time = transitionTime)
            if index == 1:
                stream.panel_set(id, GREEN[0], GREEN[1], GREEN[2], transition_time = transitionTime)
            if index == 2:
                stream.panel_set(id, BLUE[0], BLUE[1], BLUE[2], transition_time = transitionTime)
        time.sleep(speed)

def streaming_red(a, args):
    stream = a.effect_stream()
    panels = a.rotated_panel_positions['positionData']
    panel_ids = [x['panelId'] for x in panels]
